...
void main() {
  ...
  setUp(() async {
    ...
    container = ProviderContainer(
      overrides: [downloadStoreProvider.overrideWithValue(store)],
    );
    ...
    TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
        .setMockMethodCallHandler(const MethodChannel(eventChannel), (
          call,
        ) async {
    ...
    final serviceProvider = Provider<NativeDownloadManagerService>(
      (ref) => NativeDownloadManagerService(ref),
    );
  ...
  test(
    'state event flips pending → downloading and broadcasts the change',
    () async {
  ...
  test('progress event persists bytes while non-terminal', () async {
    await store.insertTask(baseTask(status: DownloadStatus.downloading));
  ...
  test('cancelled event deletes the task from Drift', () async {
    ...
    expect(
      changes.any((c) => c.newStatus == null && c.taskId == 'task-1'),
      isTrue,
    );
  });
