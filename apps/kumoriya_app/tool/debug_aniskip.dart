      ...
      for (final duration in [1440, 1380, 1320]) {
        ...
        if (segments.isNotEmpty) {
          ...
          for (final s in segments) {
            print('    RAW: ${jsonEncode(s)}');
            final skipType = s['skip_type'] ?? s['skipType'];
            final interval = s['interval'] as Map<String, dynamic>?;
            final startTime = interval?['start_time'] ?? interval?['startTime'];
            final endTime = interval?['end_time'] ?? interval?['endTime'];
            print('    → type=$skipType start=$startTime end=$endTime');
          }
          break;
        }
        if (duration == 1320) {
