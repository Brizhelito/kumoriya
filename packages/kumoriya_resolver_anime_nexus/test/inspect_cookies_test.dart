...
void main() {
  test('inspect Set-Cookie headers from stream data fetch', () async {
    ...
    final url = 'https://api.anime.nexus/api/anime/details/episode/stream'
        '?id=$episodeId'
        '&fillers=true'
        '&recaps=true';
    ...
    try {
      ...
      response.headers.forEach((key, val) {
        if (key.toLowerCase().contains('cookie') || key.toLowerCase().contains('auth') || key.toLowerCase().contains('set-')) {
          print('    $key: $val');
        }
      });
