...
void main() {
  ...
  test(
    'download player JS and extract auth contract',
    () async {
      const watchUrlStr =
      ...
      for (final m in inlineMatches) {
        ...
        if (content.contains('socket') ||
            content.contains('attest') ||
  ...
  test(
    'full flow WS auth with real services',
    () async {
      const watchUrlStr =
          'https://anime.nexus/watch/019b9e8f-edf6-71a7-87c5-c45f64297245/execution-537a058e13efbfab1729';
      ...
      final pageData = await NexusPageScraper(
        dio,
      ).scrape(watchUrl, session: browserSession);
      ...
      final streamData = await NexusStreamDataFetcher(
        dio,
      ).fetch(episodeId: pageData.episodeId, session: session2);
      ...
      print(
        '[step-3] cookie=${cookieHeader?.substring(0, (cookieHeader.length).clamp(0, 80))}...',
      );
      ...
      final authPayload = jsonEncode(<Object>[
        'auth',
        <String, String>{'ref': attestRef, 'fingerprint': fingerprint},
      ]);
