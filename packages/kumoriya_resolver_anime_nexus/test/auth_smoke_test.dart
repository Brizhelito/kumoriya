...
void main() {
  ...
  group('Auth smoke (live) –', () {
    // ------------------------------------------------------------------ [1]
    test('[1] auth session bootstrap sets cookies', () async {
    ...
    // ------------------------------------------------------------------ [2]
    test('[2] stream metadata returns data.hls (full fetch flow)', () async {
      print('\n--- [2] NexusStreamDataFetcher.fetch() ---');
      final episodeId = Uri.parse(_episodeUrl).pathSegments
    ...
    // ------------------------------------------------------------------ [3]
    test('[3] page scraper extracts attestRef', () async {
      print('\n--- [3] NexusPageScraper.scrape() ---');
      try {
        final pageData = await NexusPageScraper(dio).scrape(
          Uri.parse(_episodeUrl),
          session: session,
        );
    ...
    // ------------------------------------------------------------------ [4+5]
    test('[4+5] WS connect + auth handshake (requires [2]+[3])', () async {
      ...
      print('[4] cookieHeader=${streamData.cookieHeader != null ? "present (${streamData.cookieHeader!.length} chars)" : "MISSING ⚠️"}');
      ...
      print('[6] WS headers: Origin=${NexusConstants.mainBase} | '
      ...
      socket.listen(
        (raw) {
          ...
          if (msg.startsWith('0{') || msg == '0') {
            ...
            String sid = '';
            try {
              sid = (jsonDecode(msg.substring(2)) as Map)['sid']?.toString() ?? '';
            } catch (_) {}
        ...
        onDone: () {
          print('[ws] ⚠️  SOCKET_CLOSED closeCode=${socket.closeCode} '
      ...
      if (result != 'AUTHENTICATED') {
        ...
        print('  cookieHdr   = ${streamData.cookieHeader?.substring(0, 40) ?? "MISSING"}...');
